    ...
    if (lineDataSet.fillArea && points.isNotEmpty()) {
        ...
        if (lastNonNull != null && firstNonNull != null) {
            val fillPath = Path().apply {
                ...
                // Fill to y=0 if data includes negative values, otherwise to bottom
                val fillBaselineY = if (bounds.minY < 0 && bounds.maxY > 0) {
                    mapToScreenY(0f, bounds.minY, bounds.maxY, size.height, padding)
                } else {
                    size.height - padding
                }
                lineTo(lastX, fillBaselineY)
                lineTo(firstX, fillBaselineY)
                close()
            }
            drawPath(
                path = fillPath,
